/**
 * Created by zhufengpeixun on 14/12/31.
 */
if (!("ace" in window)) {
    window.ace = {}
}
...
jQuery(function(a) {
    ...
    ace.tab_fixed(jQuery);
    ace.winScroll(jQuery);
    ace.closer(jQuery);
...
ace.tab_fixed=function(a){
    var myTab=document.querySelector('#myTabfixed');
    if(!myTab){
       return;
    }
    var myTabHeight=parseInt(window.getComputedStyle(myTab,null)["height"]);
    var myTabOffsetParent=myTab.offsetParent;
    ace.navTop=myTab.offsetTop;

    while(myTabOffsetParent){
        if(myTabOffsetParent.nodeType==1){
            ace.navTop+=myTabOffsetParent.offsetTop;
        }
        myTabOffsetParent=myTabOffsetParent.offsetParent;

    }

    ace.navTop=ace.navTop-myTabHeight;
}

ace.winScroll= function (a) {
    if(ace.navTop=="undefined"){
        return;
    }
   window.onscroll= function () {
       ace.flag=false;

       var aScrollTop=a(window).scrollTop();
       if(aScrollTop>=ace.navTop){

               a('#myTabfixed').css({"position":"fixed","top":0});
               ace.flag=true;


       }else{
           a('#myTabfixed').css({"position":"relative","top":1});
           ace.flag=false;
       }
   }

   a('#myTabfixed li').on(ace.click_event,"a",function(){
      if(ace.flag==true){

          a(window).scrollTop(ace.navTop);
      }

   })

}
ace.closer=function(a){
    a('.closer').on(ace.click_event,function(){
        $('#contentArticle').hide();
